...
const OTPVerification = () => {
  ...
  const handleVerify = () => {
    if (cardLabel === "Login") {
      axios
        .post(`${import.meta.env.VITE_API_URL}/user/verifyloginOTP`, {
          email: email,
          otp: otp,
        })
        .then((r) => {
      ...
      axios
        .post(`${import.meta.env.VITE_API_URL}/register/verifyOTP`, {
          email: email,
          otp: otp,
        })
        .then((r) => {
          ...
          if (r.status === 200) {
            ...
            const file = new Blob([r.data.message], { type: "text/plain" });
